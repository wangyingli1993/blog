	...
	function submitData() {
		...
		if(content == null || content == "") {
			...
			$.post(
				...
				{"content":content,"imageCode":imageCode,"blog.id":"${blogItem.id}"},
				function(result) {
					if(result.success) {
	...
	<div>
		...
		<div class="blog_info">
			...
			<div style="float:right;">
				发布于：『
				<fmt:formatDate value="${blogItem.releaseDate }" type="date"
					pattern="yyyy-MM-dd HH:mm" />
				』&nbsp;&nbsp; 博客类别：<a
					href="${pageContext.request.contextPath}/index.html?typeId=${blogItem.blogType.id }">${blogItem.blogType.typeName }</a>&nbsp;&nbsp;
				阅读(${blogItem.clickHit })&nbsp;&nbsp; 评论(${blogItem.replyHit })
			</div>
		</div>
		<br><br>
		...
		<div style="line-height:3; background-color: #F8F8FF">
			<font style="color:#8B2323">作者：王颖利&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;尊重博主原创文章，转载请注明文章出于此处。</font>
		</div>
		<div class="xian" style="margin:0px auto; border-top:1px solid #ddd"></div>
		<div class="blog_content">${blogItem.content }</div>
